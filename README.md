...
TODO FRO FEB 05 MEG TEST

BITS FORMAT  FOR SAVING DATA 

move the image sequene to center overlay and the frame an the fixation
...
for each trial
    eye record
    (wait 500ms) it can be the part of green fixation cross
    800 on
double check this in next meeting as well
800 on
50 off 
800 on 
50 off 
 1000 green 
1000 all images 
target vanishes and images start moving 

moves
    eye trigger (eye message)
     meg start trigger at same time 
    ...
    trial completes
    (wait 500ms)
    also save behav timings for each triggs
    eye stop 
    save edf file

remember to run a command everytime to mirror for the meg


Feb 11 
We tested the task in eye tracking lab in fedrigotti with @alex and @giuliano. 

Todo:
Add a cue before visual and mental modes in the speed block.
For example : Fast Day 
Present the images in central location ( currently 0.5 deg above center to match target) 



Check the code on vanishing images 

Compute relevant variables and perform sanity check, 
check the duration of presentation of  trial phases match expected duration

Add commandwindow
Add hide cursor ( same as meg )
CHange trial start to movement start and not blink 
validated Psychological questionares

## TODO
Add instruction screen to manual exp as well . include the story without day/night and speed . 
