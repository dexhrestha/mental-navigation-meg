function flag = eye_beginTrialFixation(eye,params)
...
fixationWindow = [-params.eye.fixWinSizeSmallX -params.eye.fixWinSizeSmallY params.eye.fixWinSizeSmallX params.eye.fixWinSizeSmallY];
fixationWindow = fix(CenterRect(fixationWindow, params.ptb.windowRect));

% draw white diode, so there will be a bigger constrast when
% the trial starts
Screen('FillRect',params.ptb.window, [255, 255, 255],params.ptb.diode);
% draw fixation dot
Screen('FillOval',params.ptb.window, params.FIX_COLOR, ...
    [(params.ptb.xCenter-params.eye.stimSize) (params.ptb.yCenter-params.eye.stimSize) ...
    (params.ptb.xCenter+params.eye.stimSize) (params.ptb.yCenter+params.eye.stimSize)], []);
Screen('Flip',params.ptb.window); %re-flip to show what you have drawn
...
%     Screen('HideCursorHelper',params.ptb.window);
% end
...
if eye_used ~= params.eye.eye_used
    Eyelink('StopRecording');
    assert(eye_used == params.eye.eye_used, 'recording from a different eye than specified')
end
...
while true %% if the subject doesnot fixate, the trial never starts
    ...
    if  infix==1 && GetSecs()-startFixation > params.eye.fixationTime
    ...
    elseif  infix==1 && GetSecs()-startFixation > floor(params.eye.fixationTime/2) && ...
            GetSecs()-startCheck > params.eye.maxFixationTime
    ...
    elseif GetSecs()-startCheck > params.eye.maxFixationTime
